...
class Dynamic_BPE:
    def __init__(self, tokenizer, tokenizer_boundary: str = "pretokens", is_arabic: bool = False):
        """
        Args:
           tokenizer: The base tokenizer.
           tokenizer_boundary: How to define token boundaries.
           is_arabic: Flag to indicate the text is in Arabic and should use custom rules.
        """
        ...
        if not is_arabic:
            # Original punctuation tokens for Latin-based text.
            ...
            # For Arabic, use a set of Arabic punctuation characters.
            arabic_punctuations = "،؛؟"  # comma (،), semicolon (؛) and question mark (؟)
            self.punctuation_tokens = {
                token for token in tokenizer.vocab if any(c in token for c in arabic_punctuations)
            }
        self.debug = False
    ...
    def is_valid_pair(self, pair):
        ...
        if not self.is_arabic:
            # For Latin-based text, use the original spacelike check.
            ...
            # For Arabic, we skip the Latin-specific whitespace/punctuation check.
            pass

        try:
            # The "sentence" boundary remains the same.
            if self.tokenizer_boundary == "sentence":
                return (token1 not in self.special_token_map and token2 not in self.special_token_map)
            
            # Build string from token1 + token2.
            b = [CHARS_TO_BYTES[c] for c in token1 + token2]
            string_val = bytes(b).decode("utf-8")
            
            if self.tokenizer_boundary == "pretokens":
                if self.is_arabic:
                    # For Arabic, use a simple whitespace split
                    return len(string_val.strip().split()) == 1
                else:
                    return len(self.tokenizer._tokenizer.pre_tokenizer.pre_tokenize_str(string_val)) == 1
            elif self.tokenizer_boundary == "word":
                if self.is_arabic:
                    return (len(string_val.strip().split()) == 1 and 
                            token1 not in self.special_token_map and 
                            token2 not in self.special_token_map)
                else:
                    return (len(pre_tokenizers.WhitespaceSplit().pre_tokenize_str(string_val)) == 1 and 
                            token1 not in self.special_token_map and 
                            token2 not in self.special_token_map)
            elif self.tokenizer_boundary == "word_hyphen":
                if self.is_arabic:
                    cond1 = (len(string_val.strip().split()) == 1 and
                             token1 not in self.special_token_map and 
                             token2 not in self.special_token_map)
                    ...
                    cond1 = (len(pre_tokenizers.WhitespaceSplit().pre_tokenize_str(string_val)) == 1 and 
                             token1 not in self.special_token_map and 
                             token2 not in self.special_token_map)
                    if cond1 and token1 not in self.punctuation_tokens and token2 in self.punctuation_tokens:
                        return token2 == "-"
            ...
            # If decoding fails, we allow merge.
            return True
        ...
        if mmlu:
            if isinstance(batch_examples, list):
                for batch_example in batch_examples:
                    tokens = ["<s>"] + tokenize(batch_example, self.tokenizer,
                ...
                for idx, _ in enumerate(batch_examples["prompt"]):
                    tokens = ["<s>"] + tokenize(batch_examples["prompt"][idx],
                                                 self.tokenizer, max_length=max_length, truncation=True)
                    if len(tokens) > max_length:
                        tokens = tokens[:max_length]
                    unique_tokens_original.update(tokens)
                    batch_tokens.append(tokens)
            ...
            if isinstance(batch_examples, list):
                for batch_example in batch_examples:
                    tokens = (["<s>"] + tokenize(batch_example["premise"], self.tokenizer)
                              + ["</s>", "</s>"] + tokenize(batch_example["hypothesis"], self.tokenizer)
                              + ["</s>"])
                    batch_tokens.append(tokens)
                    unique_tokens_original.update(tokens)
                    if self.debug:
                        tokens_word = (["<s>"] + pretokenize(batch_example["premise"], self.tokenizer)
                                       + ["</s>", "</s>"] + pretokenize(batch_example["hypothesis"], self.tokenizer)
                                       + ["</s>"])
                        batch_word_tokens.append(tokens_word)
            else:
                for idx, _ in enumerate(batch_examples["premise"]):
                    tokens = (["<s>"] + tokenize(batch_examples["premise"][idx], self.tokenizer)
                              + ["</s>", "</s>"] + tokenize(batch_examples["hypothesis"][idx], self.tokenizer)
                              + ["</s>"])
                    batch_tokens.append(tokens)
                    unique_tokens_original.update(tokens)
                    if self.debug:
                        tokens_word = (["<s>"] + pretokenize(batch_examples["premise"][idx], self.tokenizer)
                                       + ["</s>", "</s>"] + pretokenize(batch_examples["hypothesis"][idx], self.tokenizer)
                                       + ["</s>"])
                        batch_word_tokens.append(tokens_word)
        elif mlm:
            if isinstance(batch_examples, list):
                for batch_example in batch_examples:
                    tokens = (["<s>"] + tokenize(batch_example["text"], self.tokenizer, max_length=max_length-2)
                              + ["</s>"])
                    tokens = tokens[:max_length]
                    batch_tokens.append(tokens)
                    unique_tokens_original.update(tokens)
                    if self.debug:
                        tokens_word = (["<s>"] + pretokenize(batch_example["text"], self.tokenizer)
                                       + ["</s>"])
                        batch_word_tokens.append(tokens_word)
            else:
                for idx, _ in enumerate(batch_examples["text"]):
                    tokens = (["<s>"] + tokenize(batch_examples["text"][idx], max_length=max_length-2,
                                                 truncation=True, tokenizer=self.tokenizer)
                              + ["</s>"])
                    if self.debug:
                        tokens_word = (["<s>"] + pretokenize(batch_examples["text"][idx], self.tokenizer)
                                       + ["</s>"])
                        batch_word_tokens.append(tokens_word)
    ...
    def merge_pair_with_tracking(self, best_pair, batch_tokens, token_positions, ner: bool = False, batch_word_ids: list = []):
    ...
    def tokenize_batch(self, batch_examples: LazyBatch, max_nr_merges: int = 1000, mlm: bool = False,
                       max_length: int = 1280, ner: bool = False, nli: bool = False, mmlu: bool = False):
        unique_tokens_original = set()
        ...
        batch_seq_lengths = []
        total_merges = 0
        unique_tokens_original, batch_tokens, batch_word_tokens, batch_word_ids = self.tokenize_base_case(
            batch_examples=batch_examples, mlm=mlm, max_length=max_length, ner=ner, nli=nli, mmlu=mmlu
        )
        while total_merges < max_nr_merges:
            ...
            batch_tokens, batch_word_ids = self.merge_pair(
                a=best_pair[0], b=best_pair[1], batch_tokens=batch_tokens, ner=ner, batch_word_ids=batch_word_ids
            )
    ...
    def tokenize_batch_for_seq_len(self, batch_examples: LazyBatch, max_nr_merges: int = 20000, mlm: bool = False,
                                   max_length: int = 128, ner: bool = False, nli: bool = False, mmlu: bool = False):
        batch_tokens = []
        total_merges = 0
        _, batch_tokens, _, _ = self.tokenize_base_case(
            batch_examples=batch_examples, mlm=False, max_length=max_length, ner=False, nli=False, mmlu=True
        )
        import copy
        ...
        while total_merges < max_nr_merges:
            ...
            if best_pair == "":
                ...
                break
            total_merges += 1
            batch_tokens, _ = self.merge_pair(a=best_pair[0], b=best_pair[1], batch_tokens=batch_tokens)
            if total_merges not in self.merges2seqLen:
                self.merges2seqLen[total_merges] = 0
            for _, tokenised_text in enumerate(batch_tokens):
                self.merges2seqLen[total_merges] += len(tokenised_text)
    def get_merges2seqlen_for_dataset(self, dataset, batch_size: int = 32) -> None:
        ...
        for i in tqdm(range(0, len(dataset), batch_size), desc="Encoding Dataset"):
            ...
            self.tokenize_batch_for_seq_len(
                batch_examples=batch, max_nr_merges=100000, mlm=False, max_length=max_length, ner=False, nli=False, mmlu=True
            )
